...
// TODO: PC: Make sure all functionality from {in, out}boundMessageRouter is ported over!
// 1) Look at the message type generated by InboundMessageRouter.
// 2) Take the action for that type as specified by OutboundMessageRouter.
// - Is this the best way to handle the functionality?
/**
 * Define the proper actions in response to various events.
 */
ChatServer.prototype.bindHandlers = function() {
  var self = this;
  this.on(TYPES.login, function(connection, m) {
    self.loginUser(connection, m[CONSTANTS.FIELDS.username]);
  });
  ...
  this.on(TYPES.message, function(connection, message) {
    ...
    var message = MessageUtil.buildMessageResponse(connection.chat.username, message[CONSTANTS.FIELDS.message]);
...
ChatServer.prototype.handleInboundMessage = function(connection, message) {
  ...
  // TODO: PC: Problem with emitting 'error' event that is not handled?
  if (!message) {
...
ChatServer.prototype.loginUser = function(connection, username) {
  ...
  return this.emit(TYPES.login_success, connection, MessageUtil.buildLoginSuccessResponse(connection.chat.username));
};
